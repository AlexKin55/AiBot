...
// ---------------------------------------------------------------------------
// Barge-in (перебивание робота): микрофон и динамик делят один I2S (CoreS3),
// поэтому во время озвучки робот не может слушать непрерывно. Каждые
// BARGEIN_EVERY_N_CHUNKS сыгранных чанков playbackTask делает «окно
// прослушивания»: динамик полностью выключается (AW88298 I2SEN=0, DMA стоп),
// микрофон пишет ~BARGEIN_WINDOW_MS и меряет RMS. Своего голоса робот не
// слышит — в окне динамик молчит. Если уровень >= gVadThr * BARGEIN_RMS_GAIN
// в BARGEIN_HIT_FRAMES кадрах — озвучка обрывается, очередь чистится,
// микрофон возвращается VAD, серверу уходит "PLAYBACK:INTERRUPT".
//
// Незаметность пауз: окно короткое (~80-100 мс) и приходится раз в ~2 с
// речи (BARGEIN_EVERY_N_CHUNKS) — тишина почти не слышна. Окно слегка
// удлиняет озвучку, поэтому сервер шлёт чанки чуть медленнее реального
// времени (config audio.play_speed ~0.93), иначе очередь переполняется.
// ---------------------------------------------------------------------------
#ifndef BARGEIN_ENABLED
#define BARGEIN_ENABLED 1u
#endif

// Каждый N-й сыгранный чанк завершается окном прослушивания (0 = никогда).
// 10 -> окно раз в ~2 с речи (чанк 0.2 с): реже окна = чище звук, но реакция
// на перебивание до ~2.1 с. Частые окна (N=2) резали речь на куски —
// робот «не успевал заговорить».
#ifndef BARGEIN_EVERY_N_CHUNKS
#define BARGEIN_EVERY_N_CHUNKS 10u
#endif

// Длительность окна прослушивания, мс (сверх времени на переключение I2S).
#ifndef BARGEIN_WINDOW_MS
#define BARGEIN_WINDOW_MS 80u
#endif

// Порог перебивания: gVadThr * BARGEIN_RMS_GAIN (запас поверх VAD-порога —
// остаточный звон мембраны после выключения динамика не должен считаться
// речью).
#ifndef BARGEIN_RMS_GAIN
#define BARGEIN_RMS_GAIN 1.5f
#endif

// Сколько кадров микрофона в окне должны превысить порог (0 — достаточно
// одного). Защита от одиночных щелчков/стуков.
#ifndef BARGEIN_HIT_FRAMES
#define BARGEIN_HIT_FRAMES 2u
#endif

// Время (мс) после прерывания, в течение которого входящие аудио-фреймы
// отбрасываются: сервер ещё шлёт хвост озвучки, пока не получил
// PLAYBACK:INTERRUPT. 1500 мс с запасом на RTT Wi-Fi.
#ifndef BARGEIN_DROP_MS
#define BARGEIN_DROP_MS 1500u
#endif
